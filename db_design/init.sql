...
CREATE TABLE nids_datas (
   ...
   sport INT NOT NULL,
   dport INT NOT NULL,
   fk_nids_protocol_type_id INT REFERENCES nids_protocol_types(id) ON DELETE CASCADE,
   urgent INT NOT NULL,
   hot INT NOT NULL,
   src_bytes BIGINT NOT NULL,
   dst_bytes BIGINT NOT NULL,
   data_number VARCHAR(45) NOT NULL,
   fk_nids_service_id INT REFERENCES nids_services(id) ON DELETE CASCADE,
   fk_nids_flag_id INT REFERENCES nids_flags(id) ON DELETE CASCADE,
   ...
   fk_nids_label_id INT REFERENCES nids_labels(id) ON DELETE CASCADE,
